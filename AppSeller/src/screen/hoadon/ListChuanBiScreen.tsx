...
const ListChuanBiScreen: React.FC<NavProps> = ({navigation}) => {
  ...
  const actionSearch = async (item: string) => {
    await getListInvoice(item, 1, startDate, andDate, page);
  };
  ...
  const handleGetAll = async () => {
    await getListInvoice(code, 1, startDate, andDate, page + 1);
  };
  ...
  const handleSelectStartDate = async (dateStart: Date | string) => {
    ...
    await getListInvoice(code, 1, dateStart, andDate, page);
  };
  ...
  const handleSelectEndDate = async (dateEnd: Date | string) => {
    ...
    await getListInvoice(code, 1, startDate, dateEnd, page);
  };
