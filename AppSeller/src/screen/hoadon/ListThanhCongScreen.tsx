...
const ListThanhCongScreen: React.FC<NavProps> = ({navigation}) => {
  ...
  const actionSearch = async (item: string) => {
    await getListInvoice(item, 3, startDate, andDate, page);
  };
  ...
  const handleGetAll = async () => {
    await getListInvoice(code, 3, startDate, andDate, page + 1);
  };
  ...
  const handleSelectStartDate = async (dateStart: Date | string) => {
    ...
    await getListInvoice(code, 3, dateStart, andDate, page);
  };
  ...
  const handleSelectEndDate = async (dateEnd: Date | string) => {
    ...
    await getListInvoice(code, 3, startDate, dateEnd, page);
  };
